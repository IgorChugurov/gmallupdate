...
function doIt() {
    ...
    Promise.resolve()
        .then(function () {
            return new Promise(function (resolve,reject) {
                fs.createReadStream('tatiana.csv')
        ...
        .then(function () {
            return new Promise(function (resolve,reject) {
                fs.createReadStream('jadone.csv')
                    .pipe(parse({delimiter: ','}))
                    .on('data', function(csvrow) {
                        //console.log(csvrow);
                        //do something with csvrow
                    ...
                    .on('end',function() {
                        //do something wiht csvData
                        console.log(jadone.length);
                        resolve()
                    });
            })
        })
        .then(function () {
            tatiana=tatiana.map(function (r) {
                return r[0]
            })
            //console.log(tatiana)
            let finishArray=[];
            jadone.forEach(function (r) {
                if(tatiana.indexOf(r[0])<0 && (r[0].indexOf('.ru')<0  || r[2])){
                    let o1 ={
                        /*num:i,*/
                        email:r[0],
                        phone:r[1],
            ...
            var csv = json2csv({ data: finishArray, fields: fields });

            fs.writeFile('jadoneusersNew.csv', csv, 'utf8',function(err) {
