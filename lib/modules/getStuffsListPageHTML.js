    ...
    return Promise.resolve()
        ...
        .then(function (data) {
            ...
            if(data[1] && data[1]._id){
                ...
                // проверяем фильтры есть ли у категории
                //if((!item.filters || !item.filters.length) && !req.store.template.stuffListType[stuffListType].filtersForAll){
        ...
        .then(async function () {
            ...
            if(section && section.groupStuffs){
                items.forEach(function (item) {
                    ...
                    if(item.desc){
                        let n =(section && section.charsQty)?section.charsQty:250;
                        let s = item.desc.clearTag().substring(0,n)
                        item.desc=s.substr(0, Math.min(s.length, s.lastIndexOf(" ")))+' ...';
                    }
                ...
                items.forEach(function (item) {
                    ...
                    let n =(section && section.charsQty)?section.charsQty:250;
                    //console.log("item.desc && item.desc.length>n",item.desc && item.desc.length>n,item.desc,item.desc.length)
                    if(item.desc){
                        let s = item.desc.clearTag()
                    ...
                    /*if(req.store.subDomain=='balance-yoga'){
                        console.log(item.desc)
                    }*/
                    if(item.store!=req.store._id){
                    ...
                    /*console.log(item.abonement)
                    console.log(JSON.parse(item.stuffInList))*/
                    item.uiSref='stuffs.stuff(stuff.stateObj)'
                })
            }
