...
module.exports = function getCSSForBlock(homePath,block,type,cssData,addVar) {
    return new Promise(function(resolve,reject){
        ...
        try {
            ...
            if(block.elements && typeof block.elements=='object'){
                for(var key in block.elements){
                    ...
                    for(let i=0;i<lengthStyleBlock;i++){
                        ...
                        if(key=='a' && i==1){elementMixinArgs+='null';continue}
                        let n;
                    ...
                    //console.log(key,type+block.type+addName)
                    //if(pugCache[type+block.type+addName]){
                    /*console.log(type+block.type+'class'+sCSS)
                    console.log(pugCache[type+block.type+'class'+sCSS])*/
                    if(pugCache[type+block.type+'class'+sCSS]){
                        if(key[0]=='.'){
                            key=key.substring(1)
                            let i = key.indexOf(':');
                            if(i>-1){
                        ...
                        let css=pugCache[type+block.type+'class'+sCSS]

                        //console.log('oldkey',oldkey)
                        //console.log("block['wrapclass']",block['wrapclass']+" {@include block-wrap"+block.type+oldkey+"("+elementMixinArgs+");}")
                        /*if(block.type=='slider'){
                            console.log("@mixin block-wrap"+block.type+oldkey+"("+mixinArgsDefine+',$className:null'+") {" +css+"}")
                        }*/
                    ...
                    /*if(block.type=='text' && key==='a'){
                        console.log(block['wrapclass']+" {@include block-wrap"+block.type+oldkey+"("+elementMixinArgs+");}")
                    }*/
            ...
            //console.log('type block is ',block)
            console.log(e)
        }
