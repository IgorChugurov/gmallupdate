           ...
           /* if(d.type=='groupStuffs'){
            ...
            if(d.type && d.is && d.is!='false'){
                ...
                try{
                    ...
                    /*fs.writeFile( 'public/css/hp_'+req.store.subDomain+'.css', cssData.css, function (err, data) {
                        if (err) {console.log('fs.writeFile',err)}else {console.log('public/css/.css was written')}
                    } );*/
                }catch(err){
                    console.log(err)
                }
