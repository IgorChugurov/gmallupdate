...
module.exports = function getFooter(req,templateD,cache_key,lang,section,stat,photoHost,arrVar,jadeData,cssData){
    let template ={

    }
    //console.log(req.tablet && template.footerTablet && template.footerTablet.is)
    if(req.tablet && templateD.footerTablet && templateD.footerTablet.is){
        template.footer=JSON.parse(JSON.stringify(templateD.footerTablet));
        //console.log(req.store.footerTablet)
        req.store.footer=req.store.footerTablet
    }else if(req.mobile && templateD.footerMobile && templateD.footerMobile.is){
        template.footer=JSON.parse(JSON.stringify(templateD.footerMobile))
        req.store.footer=req.store.footerMobile
    }else{
        template.footer = JSON.parse(JSON.stringify(templateD.footer))
    }
    ...
    return new Promise(function(resolve,reject){
        ...
        try{
            ...
            if(!foot.ratio){
                ...
                jadeData.dj[key].rightClass= "col-lg-4 col-md-4 col-sm-4 col-xs-12 footer-right"
            ...
            if(foot.ratio && (foot.ratio==5|| foot.ratio==2|| foot.ratio==3|| foot.ratio==1)){
                let tt = r;
                r=c;
                c=tt;
            }
