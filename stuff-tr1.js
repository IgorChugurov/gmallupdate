...
async function run(){
    const cursor = Items.find({store:'5867d1b3163808c33b590c12',archived:false}).cursor();
    await cursor.eachAsync(async function(doc) {
        let allKeys = Object.keys(doc.stock)
        let keys = [];
        for(let key in doc.stock){
            //console.log(doc.stock[key],doc.stock[key].quantity && doc.stock[key].quantity!=='0')
            if(doc.stock[key].quantity && doc.stock[key].quantity!=='0'){

                keys.push[key];
                console.log(key,doc.stock[key].quantity && doc.stock[key].quantity!=='0',keys)
            }
        }
        console.log(doc.name,keys)
        if(allKeys[0]!='notag'){
            doc.tags= doc.tags.map(t=>t.toString());

            allKeys.forEach(k=>{
                if(keys.includes(k) && !doc.tags.includes(k)){
                    doc.tags.push(k);
                    console.log(doc.name,' ',doc.artikul,' ',doc.stock[k].name)
                }
            })
...
function handle(doc, callback) {
    ...
    let url ='http://127.0.0.1:7700/api/collections/Material?page=0'
    let options = {
        url:url,
        method: 'GET',
    ...
    requestForServer.on('response', function(response) {
        ...
        response.on('end', function() {
            ...
            console.log(i,JSON.stringify(buffer))
            callback()
    ...
    return;
    console.log(doc.nameL['ru'])
    let query=JSON.stringify({stuff:doc._id.toString()})
    url+=query
...
Promise.resolve()
   /* .then(function () {
    ...
    .then( async function () {
        ...
        const cursor = Items.find({store:'5867d1b3163808c33b590c12',archived:false})
