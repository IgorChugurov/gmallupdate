...
(function(){
    angular.module('gmall.directives')
        .directive('workers',itemsDirective)
        .directive('worker',itemDirective)
    ...
    function itemsDirective(){
        return {
            ...
            controllerAs: '$ctrl',
            templateUrl: 'bookkeep/components/worker/items.html',
        }
    }
    function itemDirective(){
        return {
            ...
            controller: itemCtrl,
            controllerAs: '$ctrl',
            templateUrl: 'bookkeep/components/worker/item.html',
        }
    }
    itemsCtrl.$inject=['Worker','$stateParams','global','$q','$uibModal','$timeout','$scope','Confirm','$state','GlobalService','$http','exception']
    function itemsCtrl(Items,$stateParams,global,$q,$uibModal,$timeout,$scope,Confirm,$state,GlobalService,$http,exception){
        ...
        function getDetail(id,virtualAccount) {
            GlobalService.getDocs('Worker',id,virtualAccount)

        }

        function createItem() {
            $state.go('frame.Worker.item',{id:'new'})
        }

        activate();

        function activate() {
            return getList().then(function () {}).then(function () {});
        }
        ...
        function makeBalances() {
            $q.when()
                .then(function () {
                    var url = '/api/bookkeep/asa/makeBalances/Worker'
                    return $http.get(url)
                })
                .then(function () {
    ...
    itemCtrl.$inject=['Worker','$stateParams','global','$q','$uibModal','$timeout','$scope','Confirm','exception','$state']
    function itemCtrl(Items,$stateParams,global,$q,$uibModal,$timeout,$scope,Confirm,exception,$state){
        ...
        function createItem() {
            ...
            $q.when()
                ...
                .then(function () {
                    $state.go('frame.Worker', undefined, { reload: true })
                })
        }
    ...
    angular.module('gmall.services')
        .service('Worker', itemsService)
    itemsService.$inject=['$resource','$uibModal','$q'];
    function itemsService($resource,$uibModal,$q){
        var Items= $resource('/api/collections/Worker/:_id',{_id:'@_id'});
        return {
            getList:getList,
            getItem:getItem,
            save:Items.save,
