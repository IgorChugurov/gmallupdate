...
(function(){
    angular.module('gmall.directives')
        .directive('founders',itemsDirective)
        .directive('founder',itemDirective)
    ...
    function itemsDirective(){
        return {
            ...
            controllerAs: '$ctrl',
            templateUrl: 'bookkeep/components/founder/items.html',
        }
    }
    function itemDirective(){
        return {
            ...
            controller: itemCtrl,
            controllerAs: '$ctrl',
            templateUrl: 'bookkeep/components/founder/item.html',
        }
    }
    itemsCtrl.$inject=['Founder','$stateParams','global','$q','$uibModal','$timeout','$scope','Confirm','$state','GlobalService','$http','exception']
    function itemsCtrl(Items,$stateParams,global,$q,$uibModal,$timeout,$scope,Confirm,$state,GlobalService,$http,exception){
        ...
        function getDetail(id,virtualAccount) {
            GlobalService.getDocs('Founder',id,virtualAccount)

        }

        function createItem() {
            $state.go('frame.Founder.item',{id:'new'})
        }
        ...
        function makeBalances() {
            $q.when()
                .then(function () {
                    var url = '/api/bookkeep/asa/makeBalances/Founder'
                    return $http.get(url)
                })
                .then(function () {
    ...
    itemCtrl.$inject=['Founder','$stateParams','global','$q','$uibModal','$timeout','$scope','Confirm','exception','$state']
    function itemCtrl(Items,$stateParams,global,$q,$uibModal,$timeout,$scope,Confirm,exception,$state){
        ...
        function createItem() {
            ...
            $q.when()
                ...
                .then(function () {
                    $state.go('frame.Founder', undefined, { reload: true })
                })
        }
    ...
    angular.module('gmall.services')
        .service('Founder', itemsService)
    itemsService.$inject=['$resource','$uibModal','$q'];
    function itemsService($resource,$uibModal,$q){
        var Items= $resource('/api/collections/Founder/:_id',{_id:'@_id'});
        return {
            getList:getList,
            getItem:getItem,
            save:Items.save,
