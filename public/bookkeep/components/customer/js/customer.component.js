...
(function(){
    angular.module('gmall.directives')
        .directive('customers',itemsDirective)
        .directive('customer',itemDirective)
    ...
    function itemsDirective(){
        return {
            ...
            controllerAs: '$ctrl',
            templateUrl: 'bookkeep/components/customer/items.html',
        }
    }
    function itemDirective(){
        return {
            ...
            controller: itemCtrl,
            controllerAs: '$ctrl',
            templateUrl: 'bookkeep/components/customer/item.html',
        }
    }
    itemsCtrl.$inject=['Customer','$stateParams','global','$q','$uibModal','$timeout','$scope','Confirm','$state','GlobalService','$http','exception']
    function itemsCtrl(Items,$stateParams,global,$q,$uibModal,$timeout,$scope,Confirm,$state,GlobalService,$http,exception){
        ...
        function getDetail(id,virtualAccount) {
            GlobalService.getDocs('Customer',id,virtualAccount)

        }

        function createItem() {
            $state.go('frame.Customer.item',{id:'new'})
        }
        ...
        function makeBalances() {
            $q.when()
                .then(function () {
                    var url = '/api/bookkeep/asa/makeBalances/Customer'
                    return $http.get(url)
                })
                .then(function () {
    ...
    itemCtrl.$inject=['Customer','$stateParams','global','$q','$uibModal','$timeout','$scope','Confirm','exception','$state']
    function itemCtrl(Items,$stateParams,global,$q,$uibModal,$timeout,$scope,Confirm,exception,$state){
        ...
        function createItem() {
            ...
            $q.when()
                ...
                .then(function () {
                    $state.go('frame.Customer', undefined, { reload: true })
                })
        }
    ...
    angular.module('gmall.services')
        .service('Customer', itemsService)
    itemsService.$inject=['$resource','$uibModal','$q'];
    function itemsService($resource,$uibModal,$q){
        var Items= $resource('/api/collections/Customer/:_id',{_id:'@_id'});
        return {
            getList:getList,
            getItem:getItem,
            save:Items.save,
