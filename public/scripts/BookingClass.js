...
(function () {
    var BookingCLass = function () {
        ...
        _createClass(BookingCLass, [{
            ...
            value: function getBookingWeekScheldule(data, lang, workplaces) {
                var wplength = 1;
                var workplacesO = {};
                    ...
                    for (var i = e.start; i < e.start + e.qty; i++) {
                        ...
                        if (i == e.start) {
                            if (workplace.week[e.date].entryTimeTable[i].usedTime) {
                                ...
                                var _o = {};
                                _o.usedTime = this.getUsedTime(e.start, e.qty);
                                _o.userId = e.user._id;
                                _o.service = e.stuffNameL && e.stuffNameL[lang] ? e.stuffNameL[lang] : e.stuffName;
                                _o.serviceLink = e.stuffLink;
                                _o.masterLink = '/master/' + e.masterUrl;
                                _o.masterName = e.masterNameL && e.masterNameL[lang] ? e.masterNameL[lang] : e.masterName;
                                _o.masters = e.masters ? e.masters : null;
                                _o.new = true;
                                _o.qty = e.qty;
                                _o.used = e.used;
                                _o.confirm = e.confirm;
                                _o.comment = e.comment;
                                _o.zIndex = 1;
                                e.usedTime = workplace.week[e.date].entryTimeTable[i].usedTime;
                                _o.closed = e.closed;
                                _o.stringify = JSON.stringify(e);
                                workplace.week[e.date].entryTimeTable[i].entries.push(_o);
                            } else {
                ...
                for (var d in workplace.week) {
                    ...
                    if (wplength > 1) {
                        for (var _ii = 0; _ii < wplength; _ii++) {
                            workplace.week[d].entryTimeTableW[workplaces[_ii]._id] = workplace.week[d].entryTimeTableW[workplaces[_ii]._id].filter(function (part) {
                                if (timePartsI.indexOf(part.i) < 0) {
                                    return;
                                }
                                return part.i >= startTimeParts && part.i < endTimeParts;
