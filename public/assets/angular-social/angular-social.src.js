        ...
        return {
            ...
            controller: ['$scope', '$q', '$http', function ($scope, $q, $http) {
                ...
                this.getCount = function (options) {
                    ...
                    //console.log(options.counter.get)
                    if (showcounts) {
                        if (options.counter.get) {
                            console.log('ffff',def.resolve)
...
app.directive('ngSocialFacebook', ['$parse', function ($parse) {
  ...
  var options = {
    counter: {
      url: '//graph.facebook.com/v2.0/fql?q=SELECT+total_count+FROM+link_stat+WHERE+url%3D%22{url}%22' +
  ...
  var options = {
    counter: {
      ...
      get: function (jsonUrl, deferred, $http) {
        ...
        options._ = deferred;
          jsonUrl+='&callback=window.services.gplus.cb'
          console.log(jsonUrl)
