...
(function(){
    ...
    function gmallSliderPageDirective($timeout,$rootScope){
        return {
            restrict:"A",
            replace:false,
    ...
    function gmallSliderHomeDirective($timeout,global){
        return {
            ...
            link:function(scope,element,attr){
                ...
                scope.innerDiv = element.find('.mytoggle');
                scope.substrate;
                scope.foo={}
                ...
                scope.links=[];
                scope.lastLink=[];
                scope.innerDiv.each(function (i,div) {
                    scope.imgs.push($(div).attr('id'));
                    scope.links.push($(div).find('a'));
                    if(!i){
                        $(div).css('opacity', 1)
                    }
                })
                ...
                try{
                    scope.lastLink=scope.links[scope.links.length-1]
                ...
                $timeout(function(){
                    ...
                    scope.innerDiv.each(function (i,img) {
                ...
                element.find('.substrate').each(function(i,div){
                    scope.substrate=$(div);
                    //console.log(scope.substrate.find('img').length)
                    scope.substrate.find('img').each(function(){
                       //console.log(this.complete)
                        var self=this;
                        if(!this.complete){
                            $(this).bind('load', function() {
                ...
                function next() {
                    ...
                    //$timeout.cancel(timer)
                    var oldIdx=scope.index;
                    $('#'+slideMark[scope.index]).removeClass('active')
                    $('#'+scope.imgs[scope.index]).css('opacity',0)
                    scope.index < scope.innerDiv.length - 1 ? scope.index++ : scope.index = 0;

                    $('#'+scope.imgs[scope.index]).css('opacity',1)
                    $('#'+slideMark[scope.index]).addClass('active')
                    try{$(scope.lastLink).attr('href',scope.links[scope.index])}catch (err){console.log(err)}
                ...
                function prev(){
                    ...
                    scope.index == 0 ? scope.index=scope.innerDiv.length - 1  : scope.index--;
                    console.log('prev',scope.index)
                    $('#'+scope.imgs[scope.index]).css('opacity',1)
                    console.log($('#'+scope.imgs[scope.index]))
                    $('#'+slideMark[scope.index]).addClass('active')
                ...
                function sliderFunc() {
                    ...
                    scope.timer = $timeout(function() {
                        //console.log(timer)
                        console.log(scope.number,scope.timer)
                        $timeout.cancel(scope.timer)
                        console.log(scope.number,scope.timer)
                        next();
                        sliderFunc()
                ...
                scope.zoomSliderImg=function(i,images){
                    ...
                    global.get('functions').val.zoomImg(scope.index,images)
                }
                var slideDelay;
                scope.prevHomePageSlide=function(){
                ...
                scope.nextHomePageSlide=function(){
                    if(slideDelay){return}
                    slideDelay=true
                    $timeout(function () {
                        slideDelay=false
