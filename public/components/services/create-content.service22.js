        ...
        function order(order,invoice){
            ...
            if(!invoice){
                ...
                if(global.get('user').val.profile.fio){
                    s +='<p> Имя  - '+global.get('user').val.fio+'</p>';
                }
                if(global.get('user').val.profile.phone){
                    s +='<p> Телефон - '+global.get('user').val.phone+'</p>';
                }
                if(global.get('user').val.profile.city){
                    s +='<p> Город  - '+global.get('user').val.city+'</p>';
                }
            }
            s +='<table width="100%" cellspacing="0" cellpadding="5" border="1px">';
            s+= '<thead><tr><th>#</th><th>наименование</th><th class="text-center">разновидности</th><th class="text-center">цена</th>';
            ...
            return '<!DOCTYPE html><html><head>' +
                '<link rel="stylesheet" type="text/css" href="http://modaland/bower_components/bootstrap/dist/css/bootstrap.css" />' +
