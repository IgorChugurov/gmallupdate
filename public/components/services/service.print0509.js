...
angular.module('gmall.services')
    .factory('CreateContent', ['global',function(global){
        ...
        function getHeader(user) {
            var s=
                '<table width="900px" cellpadding="0" cellspacing="0" style="max-width:900px;color: #333333;border-collapse:collapse; border:none;table-layout: fixed; padding: 0;margin: 0" border="0">'+
                '<tr width="100%" style="max-width:900px;">' +
                    // лого и название
                '<td width="50%" style=" padding-bottom: 40px;padding-top:40px"><a href="'+global.get('store').val.link+'">';
            ...
            s+='</a></td>';
            // телефон и емейл
            s+='<td width="50%"  style="text-align: right; padding-bottom: 40px;padding-top:40px">'
            if(global.get('store').val.seller.phone) {
                s+='<span>' +global.get('langOrder').val.phone+ '</span>'+
                    ': <a href="tel:'+global.get('store').val.seller.phone+'"><span>' +global.get('store').val.seller.phone + '</span></a></br>'
            }
            if(global.get('store').val.feedbackEmail) {
                s += '<span>e-mail</span>'+
                    ': <a href="mailto:'+global.get('store').val.feedbackEmail+'"><span>' + global.get('store').val.feedbackEmail + '</span></a>'
            }

            s+='</td></tr>';
            //переходы на сайт
            if(global.get('sections') && global.get('sections').val && global.get('sections').val[0]){
                s+='<table width="900px" cellpadding="0" cellspacing="0" style="max-width:900px;background-color: #000;border-collapse:collapse; border:1px solid #000;table-layout: fixed; padding: 0;margin: 0">' +
        ...
        function getFooter(){
            var s='</br><table width="900px" cellpadding="0" cellspacing="0" style="color: #333333;border-collapse:collapse; border:none;table-layout: fixed; padding: 0;margin: 0" border="0">'+
        ...
        function order(order,invoice,commentPrint){
            ...
            texts.mailName[lang] // это обращение
            texts.orderMailText[lang]
            ...
            var status = (global.get('langOrder').val.entered)?global.get('langOrder').val.entered.toUpperCase():'поступил';
            if(order.status==2){
            ...
            '<tr  width="100%"><td colspan="2" style="padding: 20px;text-align: center"><h3>'+user+'</h3></td></tr>';
            if(order.status==1){
                '<tr  width="100%"><td colspan="2" style="padding: 20px"><p>'+global.get('store').val.texts.orderMailText[lang]+'</p></td></tr>';
            }

            s +='<tr style="max-width:900px;"><td width="50%" style="max-width:900px;" style="padding: 5px; font-size: 20px;">'+
                status+'</br>'+
                '<h3>'+global.get('langOrder').val.order.toUpperCase()+' № '+order.num+'</h3>'+
                global.get('langOrder').val.from+' '+moment(order.date).format('lll')+'</td>';
            s += '<td style=" padding: 5px; font-size: 20px;">'

            if(order.seller.name){
                s+='<p>'+global.get('langOrder').val.seller+'<strong>'+order.seller.name+'</strong></p>';
            }
            if(invoice && order.payInfo){
                s +='<p>'+global.get('langOrder').val.forpayment+'</p>';
                s +='<p>'+order.payInfo+'</p>';
            }
            // данные покупателя
            s +='<p>'+global.get('langOrder').val.customerdata+'</p>';
            ...
            if(commentPrint && order.comment){
                s +='<div class="col-lg-6"><h3>'+global.get('langOrder').val.comments+'</h3><p>'+order.comment+'</p></div>';
            }
            ...
            s+= '<thead><tr><th>#</th><th>'+global.get('langOrder').val.title+'</th><th class="text-center">'+global.get('langOrder').val.species+'</th><th class="text-center">'+global.get('langOrder').val.price+'</th>';
            s+='<th class="text-center">'+global.get('langOrder').val.quantity+'</th><th class="text-center">'+global.get('langOrder').val.sum+'</th></tr></thead>';
            ...
            s +='<tr><th colspan="4">'+global.get('langOrder').val.sum+'</th>'+
                '<th class="text-center">'+order.getTotalQuantity()+'</th><th class="text-center">'+(order.kurs*((order.sum0)?order.sum0:order.sum)).toFixed(2)+' '+order.currency+'</th></tr>';
            if(order.discount){
                s +='<tr><th colspan="4">'+global.get('langOrder').val.withdiscount+'</th>';
            ...
            if(order.coupon && order.coupon._id){
                s +='<tr><th colspan="4">'+global.get('langOrder').val.basedcoupon+'</th><th></th>'+
