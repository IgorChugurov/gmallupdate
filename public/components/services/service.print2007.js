...
angular.module('gmall.services')
    .factory('CreateContent', ['global',function(global){
        ...
        function emailFromNews(item){
            var s=
                '<table width="100%" cellpadding="0" cellspacing="0" style="color: #333333;border-collapse:collapse; border:none;table-layout: fixed; padding: 0;margin: 0" border="0">'+
                '<tr width="100%"><td width="100%" style="text-align: center; padding: 20px 50px"><img alt="" style="width: 70px;display:block;" src="'+photoHost+'/'+global.get('store').val.logo+'"></td></tr>'+
            ...
            if(item.img){
                s+='<tr width="100%"><td  width="100%" style=" padding: 20px 50px" ><a href="" style="cursor: pointer;">'+
                '<img alt="" style="width: 100%;margin-bottom: 20px;max-width: 800px; display: block" src="'+photoHost+'/'+item.img+'"></a></td></tr>';

            }
            s+='<tr width="100%"><td style=" padding: 20px 50px"><p style="color: #333333; font: 13px Arial, sans-serif; line-height: 30px; -webkit-text-size-adjust:none;">'+item.desc+'</p></td></tr>';
            if(item.video){
                s+='<tr><td width="100%"  style=" padding: 20px 50px">'+
                    item.video+
                    '</td></tr>';
            }
            s+='<tr width="100%"><td style=" padding: 20px 50px"><p style="text-align: justify;  color: :#333333; font: 13px Arial, sans-serif; line-height: 30px; -webkit-text-size-adjust:none;">'+item.desc1+'</p></td></tr></table>'+
            '<table width="100%" cellpadding="0" cellspacing="0" style="color: #333333; border-collapse:collapse; border:none;table-layout: fixed; padding: 0;margin: 0" border="0">';
            for (var i=0,l=item.imgs.length;i<l;i += 2){

                //console.log(item.imgs[i])

                s+='<tr><td style="padding: 20px 50px;">'+
                    '<a href="'+item.imgs[i].link+'"> <img alt=""  style="width: 100%; display: block"   src="'+photoHost+'/'+item.imgs[i].img+'"> </a></td>';
                 if(item.imgs[i+1])   {
                     s+='<td style="padding: 20px 50px;">'+
                         '<a href="'+item.imgs[i+1].link+'"> <img  alt="" style="width: 100%; display: block"  src="'+photoHost+'/'+item.imgs[i+1].img+'"> </a></td>';
                 }
            ...
            s+='<table width="100%" cellpadding="0" cellspacing="0" style="color: #333333;border-collapse:collapse; border:none;table-layout: fixed; padding: 0;margin: 0" border="0">'+
            '<tr><td style=" padding: 20px 50px"><p style="color: color: #333333; font: 13px Arial, sans-serif; line-height: 30px; -webkit-text-size-adjust:none;">'+item.desc2+'</p></td></tr></table>';
            s+='<table width="100%" cellpadding="0" cellspacing="0" style="color: #333333; border-collapse:collapse; border:none;table-layout: fixed; padding: 0;margin: 0" border="0">';
            for (var i=0,l=item.stuffs.length;i<l;i += 2){
                s+='<tr><td style="padding: 20px 50px; text-align: center;vertical-align: top">'+
                    ...
                    '<span style="font-size: 18px; color: #333333; font: 13px Arial, sans-serif; line-height: 30px; -webkit-text-size-adjust:none;">'+item.stuffs[i].name+'</span></a></td>';
                if(item.stuffs[i+1])   {
                    s+='<td style="padding: 20px 50px;text-align: center;vertical-align: top">'+
                        '<a alt="" href="'+item.stuffs[i+1].link+'"> <img  style="width: 100%; display: block"  src="'+photoHost+'/'+item.stuffs[i+1].gallery[0].thumb+'"> ' +
                        '<span style="font-size: 18px; color: #333333; font: 13px Arial, sans-serif; line-height: 30px; -webkit-text-size-adjust:none;">'+item.stuffs[i+1].name+'</span></a></td>';
                }
                s+='</tr>'
            }
           /* item.stuffs.forEach(function(stuff){
            ...
            s+='</table>';
            s+='<table width="90%" cellpadding="0" cellspacing="0" style="color: #333333;border-collapse:collapse; border:none;table-layout: fixed; padding: 0;margin: 0 5%" border="0">'+
                '<tr><td border="0" colspan="4" style="border:none; border-top:#cccccc 5px solid;"></td></tr>'+
                ...
                '<tr><td align="left"><span style="font-family:Tahoma; font-size:12px; color:#404040;">domain.com (c) 2011<br/>' +
                '<a href="%unsucscribe_link%" target="_blank" style="font-family:Tahoma; font-size:12px; color:#404040; text-decoration:none;">unsucsribe</a> from all lists</span><td/></td><td/>' +
                '<td align="right"><span style="font-family:Tahoma; font-size:12px; color:#404040;">';
