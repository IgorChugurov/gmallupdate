...
var  configur = require('../../lib/controllers/config.js');
/*config.init('./data/config.json', function(resp) {
    if (resp != 0) {
        console.log('Could not load config file.');
        return;
    }
    console.log('host: ' + config.parse.az);
    console.log('port: ' + config.pasre.scars);
});*/
...
exports.getConfig= function(req, res) {
  /*  if (require.cache['../../data/config.json']){
     console.log('delete');
     delete require.cache[path];
     }
...
*/
    configur.init('./data/config.json', function(resp) {
        if (resp == -1) {
            return res.json({error:'Could not load config file.'});
        }
