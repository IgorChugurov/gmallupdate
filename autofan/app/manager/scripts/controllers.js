...
//**********************************************************************************************
    ...
    .controller('customCtrl',
      ['$scope','$rootScope','$resource','$uibModal','$http','$q','$timeout',
        function ($scope,$rootScope,$resource,$uibModal,$http,$q,$timeout) {
        ...
        var Producer = $resource('http://46.101.131.133:7700/api/collections/Producer/:id',{id:'@_id'});
          var Work = $resource('http://46.101.131.133:7700/api/collections/Work/:_id',{_id:'@_id'});

        $scope.$ctrl={}
        $scope.$ctrl.works=[]
        $scope.$ctrl.refreshWorks=function (searchStr) {
          $scope.$ctrl.newWorkName = searchStr
          if(!searchStr){return}
            $q.when()
              .then(function(){
              ...
              .then(function (res) {
                res.shift();
                $scope.$ctrl.works=res;
              })
              .catch(function (err) {
                console.log(err)
              })
        }
          $scope.$ctrl.tagTransform = function (newTag) {
            $scope.$ctrl.disableSeachWork = true;
            var item = {
              name: newTag,
              workingHour:1
            };
            $q.when()
              .then(function(){
                console.log(item)
                return
                return Work.save(item).$promise;
              })
              .then(function (res) {
                  item._id=res._id;
                $scope.$ctrl.disableSeachWork=false;
                //$scope.$ctrl.refreshWorks($scope.$ctrl.newWorkName)
              })
              .catch(function (err) {
                $scope.$ctrl.disableSeachWork=false;
              })
        ...
        $scope.$ctrl.addWorkInBookkeep=function(){
            $scope.$ctrl.disableSeachWork = true;
            if($scope.$ctrl.newWorkName && $scope.$ctrl.workingHour){

            }else{
              $scope.$ctrl.disableSeachWork=false;
                alert('введите нормочас')
            }
        }
        $scope.$ctrl.changeWorks=function () {
          if ($scope.$ctrl.work){
            $scope.jobTicket.jobs[$scope.jobTicket.jobs.length]={
                name:$scope.$ctrl.work.name,
                norma:$scope.$ctrl.work.workingHour,q:1,date:null,sum:null,master:null,jobType:null,
                forSave:true
            }
          }
        }
        ...
        $q.when()
            .then(function () {
                return Producer.query({perPage:500 ,page:0,lang:'ru',store:'5a3cc10e1626aa0566f7ea87'} ).$promise
            ...
            if (confirm("Закрыть наряд-заказ?")){
                ...
                for (var i= 0,l=$scope.jobTicket.sparks.length;i<l;i++){
                    if (!$scope.jobTicket.sparks[i].forSave){
                        ...
                        if(!$scope.jobTicket.sparks[i].supplier){
                            return alert('для запчасти '+$scope.jobTicket.sparks[i].name+'('+$scope.jobTicket.sparks[i].code+')'+'не установлен поставщик')
                        }
                    }

                }

                /*$scope.jobTicket.sparks.forEach(function (sp) {
                    if(!sp.supplier){
...
*/
        ...
        function setIncomePrice(){
            var options ={
                ...
                resolve: {
                    ...
                    producers: function(){return $scope.producers}
                }
            }
        ...
        function setIncomePriceCtrl($scope,$uibModalInstance,$resource,sparks,producers){
            //console.log($rootScope.config.currency['EUR'][0])
...
//**********************************************************************************************
    ...
    .controller('jobTicketArchViewCtrl',['$scope','$rootScope','$resource','$timeout','$q','$http',function($scope,$rootScope,$resource,$timeout,$q,$http){
        var JobTicketArch = $resource('api/jobticketarch/:id',{id:'@_id'});
        var Shipper = $resource('api/shipper/:id',{id:'@_id'});
        ...
        $scope.createByAPI=function () {
            ...
            zakaz.comment='Заказ открыт '+moment(data.date).format('LLL')+' закрыт '+moment(data.dateClose).format('LLL')
            if(data.mile){
            ...
            data.jobs.forEach(function (job) {
                var w ={
                    worker:$scope.getNameWorker(job.worker),
            ...
            if(data.worker){
                data.worker=$scope.getNameWorker(data.worker)
            }
            ...
            $q.when()
                ...
                .then(function (res) {
                    console.log(res)
                    if(res.data){
                        $scope.jobTicket.createByAPI= res.data.createByAPI;
                        JobTicketArch.save($scope.jobTicket)
                    }
                    alert('все ok')
                    //console.log(res)
                })
    ...
    .controller('summaryCtrl', ['$scope','$rootScope','$timeout','$resource',
        function ($scope,$rootScope,$timeout,$resource) {
            ...
            $scope.getList = function(){
                ...
                JobTicketArch.query({dtfrom:start.toString(),dtto:end.toString(),perPage:1000,data:'all'},function(res){
                    //console.log(res)
                    if (res.length>0){
                        res.shift();
                    }
            ...
            $scope.getNameShipper = function(shipper){
                //console.log($scope.shippers)
                for (var i= 0,l=$scope.shippers.length;i<l;i++){
                    if ($scope.shippers[i]._id==shipper){
                        return $scope.shippers[i].name;
                        break;
                    }
                }
                return shipper
